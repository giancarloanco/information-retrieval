...
def find_nearest_vector(index_central_vector, vectors):
    ...
    output_list = np.sort(results, axis=0) #np.argsort(results[:,0]) 
    output_list = output_list[::-1]
    
    return index_nearest_vectors, np.amax(results[:,0]), output_list
...
index_nearest_vectors, value_nearest_vector, ordered_vectors = find_nearest_vector(0, list_vectors)
